...
* **[prototype](./prototype/SKILL.md)** — 构建一次性原型以回答设计问题：用于状态/逻辑的单个可共享 HTML 文件，或几个可切换的 UI 变体。

* **[diagnosing-bugs](./diagnosing-bugs/SKILL.md)** — 针对顽固 Bug 和性能回归的受控诊断循环：构建针对此 Bug 变红的反馈循环 → 最小化 → 假设 → 检测 → 修复 → 回归测试。
...
* **[code-review](./code-review/SKILL.md)** — 对自固定点以来的差异进行双轴审查：**标准**（是否遵循仓库的编码标准，加上 Fowler 嗅觉基线？）和 **规范**（是否忠实实现原始问题/规范？），作为并行子代理运行。

* **[resolving-merge-conflicts](./resolving-merge-conflicts/SKILL.md)** — 逐一解决进行中的 git 合并或变基冲突块，通过追溯到每一方主要来源的意图来解决，然后完成操作——绝不 `--abort`。

* **[wizard](./wizard/SKILL.md)** — 生成一个交互式 bash 向导，引导人类完成他们只能执行的步骤：配置基础设施，设置凭据或 CI 密钥，浏览不熟悉的三方仪表板，或运行一次性迁移或切换。
