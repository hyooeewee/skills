...
> 解释：对于此仓库，“问题跟踪器”是问题所在位置。像 `to-tickets`、`triage`、`to-spec` 和 `qa` 这样的技能从中读取和写入 — 它们需要知道是调用 `gh issue create`，在 `.scratch/` 下写入 Markdown 文件，还是遵循你描述的任何其他工作流。选择你实际跟踪此仓库工作的地方。
