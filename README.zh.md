...
关键在于，[`/improve-codebase-architecture`](./skills/engineering/improve-codebase-architecture/SKILL.md) 帮助你挽救一个已经变成烂摊子的代码库。我建议每隔几天对你的代码库运行一次该技能。
...
These split on one axis — who can invoke them. **用户调用** skills are reachable only when you type them (e.g. `/grill-me`); their job is to orchestrate. **模型调用** skills can be invoked by you *or* reached for automatically by the agent when the task fits; they hold the reusable discipline. A user-invoked skill may invoke model-invoked skills, but never another user-invoked one.
...
* **[wayfinder](./skills/engineering/wayfinder/SKILL.md)** — 将一大块工作（超过一个代理会话的容量）规划为问题跟踪器上调查票据的共享地图——逐一解决它们，直到通往目的地的路径清晰。

**模型调用**

* **[prototype](./skills/engineering/prototype/SKILL.md)** — 构建一个一次性原型来回答设计问题——可运行的终端应用程序用于状态/逻辑问题，或可从一个路由切换的几个截然不同的 UI 变体。
* **[diagnosing-bugs](./skills/engineering/diagnosing-bugs/SKILL.md)** — 用于顽固 Bug 和性能回归的有纪律诊断循环：重现 → 最小化 → 假设 → 注入监控 → 修复 → 回归测试。
...
* **[code-review](./skills/engineering/code-review/SKILL.md)** — 对自固定点以来的差异进行双轴审查：**标准**（它是否遵循仓库的编码标准，加上 Fowler 味道基线？）和**规范**（它是否忠实地实现原始问题/PRD？），作为并行子代理运行，以免相互干扰。
...
* **[grill-me](./skills/productivity/grill-me/SKILL.md)** — 无休止地被审问计划和设计，直到决策树的每个分支都得到解决。
...
* **[writing-great-skills](./skills/productivity/writing-great-skills/SKILL.md)** — 关于如何写好和编辑技能的参考：使技能具有可预测性的词汇和原则。

**模型调用**

* **[grilling](./skills/productivity/grilling/SKILL.md)** — 无休止地审问用户关于计划、决策或想法，直到决策树的每个分支都得到解决。`grill-me` 和 `grill-with-docs` 背后的可重用循环。
